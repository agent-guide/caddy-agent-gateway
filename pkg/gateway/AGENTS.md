...
## Route id convention (`mcproute`, `acproute`, `builtinroute`, `agentroute`)
...
- optional `rate_limits` policies enforce in-memory request token buckets for
  LLM, MCP, and agent ingress; agent policy produces independent ACP, builtin,
  and unified-agent buckets. During the internal M4-to-M5 overlap, reaching one
  runtime through both its legacy and unified ingress can therefore consume up
  to twice the configured Agent allowance; M5 removes the legacy ingress rather
  than preserving that overlap. Admission runs centrally after VirtualKey validation and
