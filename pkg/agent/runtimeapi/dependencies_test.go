...
func TestRuntimeAPIImportsOnlyAgentContract(t *testing.T) {
	t.Parallel()

	root := filepath.Join("..", "..", "..", "pkg", "agent", "runtimeapi")
	err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		...
		for _, imported := range file.Imports {
			...
			if isStandardLibraryImport(importPath) ||
				...
				importPath == "github.com/agent-guide/agent-gateway/pkg/agent/runtimeapi" {
				continue
			}
			line := files.Position(imported.Pos()).Line
	...
	if err != nil {
		t.Fatalf("inspect runtimeapi imports: %v", err)
	}
}
