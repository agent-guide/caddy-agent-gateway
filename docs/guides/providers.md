...
- `claudecode` accepts either an Anthropic-style `api_key` or a managed `oauth_token`. A managed `oauth_token` and any `sk-ant-oat-` OAuth token always use `Authorization: Bearer`. A plain API key follows `option api_key_header`, which defaults to `authorization` (`Authorization: Bearer`); set it to `x-api-key` to send the key in the `x-api-key` header instead. Use `option compact codex` when routing Codex CLI traffic through an upstream that gates on Claude Code tool names: it rewrites Codex tool names (e.g. `exec_command`) to their Claude Code equivalents (e.g. `Bash`) on the outbound request and restores the original names on the response, including native raw tool definitions. The provider preserves regular and redacted Anthropic thinking blocks, native server-tool definitions and results, citation stream events, and opaque history needed by later turns. Logical-model routes filter out providers that cannot preserve this Anthropic-native state before credential selection; direct routes return `501 Not Implemented` before credential selection when their provider lacks the capability. This is a deliberate affinity rule: a history containing authentic Anthropic signatures, citations, or otherwise unmodeled blocks is not downgraded or replayed across an incompatible provider, even if that route previously handled generic messages. The eino-backed `anthropic` provider does not preserve web-search response blocks, so it lacks the full native-content capability, but it remains eligible for authentic signed/redacted reasoning through the narrower Anthropic reasoning-replay capability. The routed provider writes the active native/reasoning dialect requirements at Debug level when affinity restricts fallback. Custom client tools must provide an object-shaped `input_schema`; an absent schema is normalized to an empty object schema, while malformed and non-object schemas are rejected as invalid requests rather than silently degraded. On logical-model routes, client tools also require resolved managed-model capability metadata with `tools: true`; Anthropic server tools are filtered by native dialect support instead. The provider also sends the full Claude Code CLI client fingerprint (`User-Agent`, `X-App`, `X-Stainless-*`, `anthropic-version`, `anthropic-beta`) by default, so a config that omits these still presents as the standard CLI; set `network.extra_headers` only to override individual values when matching a newer CLI release. Provider-level `context_window`, `max_output_tokens`, `default_max_tokens`, and `vision` options allow model-specific compatible endpoints to override the generic defaults; per-model differences should still use managed-model `capability_overrides` when one provider exposes heterogeneous models.
